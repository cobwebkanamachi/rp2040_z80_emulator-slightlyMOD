...
void loadBasic(void) {
  int i;
  Serial.println("\n\rBooting from flash:");
  zprintf("Loading: BIOS  at 0x%04x ", 0);
  for (i = 0; i < sizeof(init8250); i++) {
    RAM[i] = init8250[i];
  }
  zprintf("%5d bytes... Done\n\r", sizeof(init8250));
  zprintf("Loading: Basic at 0x%04x ", 0x150);
...
void SDfileOpen() {
  ...
  if (!f) {
    ...
    zprintf("%7d Bytes %5d Block(s)", f.size(), blocks);
  }
  Serial.println();
  f.close();
...
void FileToRAM(char c[], uint16_t l) {
  digitalWrite(LED_BUILTIN, HIGH);
  dled = true;
  zprintf("Loading: %s at 0x%04x ", c, l);
  File f;
